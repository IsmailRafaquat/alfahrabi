...
export class ShopProductCategoriesComponent implements OnInit {
  ...
  loadList(reset = false): void {
    ...
    this.loading = true;
    this.service.getList({ filter: this.search || undefined, parentCategoryId: this.parentFilter && this.parentFilter !== 'root' ? this.parentFilter : undefined,
  ...
  remove(row: ShopProductCategoryDto): void {
    this.confirmation.warn('::ConfirmDeleteProductCategory', row.name).subscribe(status => { if (status !== Confirmation.Status.confirm) return;
