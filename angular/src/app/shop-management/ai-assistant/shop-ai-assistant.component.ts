...
type AiCardKind = 'guided' | 'preview' | 'module' | 'field' | 'ambiguous' | 'missing' | 'dataList' | 'result' | 'none';

/** Maps a list-read action to where "Open <Module>" should navigate and which permission gates that button. */
const DATA_LIST_OPEN_ROUTES: Partial<Record<ShopAiActionType, { path: string; policy: string }>> = {
  [ShopAiActionType.GetUnits]: { path: '/shop-management/units', policy: 'ShopManagement.Units' },
  [ShopAiActionType.GetProductCategories]: { path: '/shop-management/product-categories', policy: 'ShopManagement.ProductCategories' },
  [ShopAiActionType.GetProducts]: { path: '/shop-management/products', policy: 'ShopManagement.Products' },
  [ShopAiActionType.GetCustomers]: { path: '/shop-management/customers', policy: 'ShopManagement.Customers' },
  [ShopAiActionType.GetSuppliers]: { path: '/shop-management/suppliers', policy: 'ShopManagement.Suppliers' },
  [ShopAiActionType.GetExpenseCategories]: { path: '/shop-management/expense-categories', policy: 'ShopManagement.ExpenseCategories' },
  [ShopAiActionType.GetBankAccounts]: { path: '/shop-management/bank-accounts', policy: 'ShopManagement.BankAccounts' },
  [ShopAiActionType.GetSales]: { path: '/shop-management/sales', policy: 'ShopManagement.Sales' },
  [ShopAiActionType.GetPurchaseOrders]: { path: '/shop-management/purchase-orders', policy: 'ShopManagement.PurchaseOrders' },
  [ShopAiActionType.GetExpenses]: { path: '/shop-management/expenses', policy: 'ShopManagement.Expenses' },
  [ShopAiActionType.GetStockAdjustments]: { path: '/shop-management/stock-adjustments', policy: 'ShopManagement.StockAdjustments' },
  [ShopAiActionType.GetPhysicalStockCounts]: { path: '/shop-management/stock-counts', policy: 'ShopManagement.StockCounts' },
};
...
export class ShopAiAssistantComponent implements OnInit, OnDestroy {
  ...
  cardKind(msg: ChatDisplayMessage): AiCardKind {
    ...
    // Checked before 'module' - a list/count read action must never fall through to the
    // module-help card even though both can technically carry a moduleKey.
    if (msg.responseType === ShopAiResponseType.DataList && msg.dataList) return 'dataList';
  ...
  // ------------------------------------------------------------------
  // Data list (existing-records) card
  // ------------------------------------------------------------------

  dataListColumns(msg: ChatDisplayMessage): string[] {
    const firstRow = msg.dataList?.rows?.[0];
    // "id" is carried in each row for potential future use (e.g. row navigation) but is never a
    // useful column for a user to see - every row shape includes it, so filtering it here keeps
    // every list (Units, Products, Customers, ...) free of the raw internal Guid.
    return firstRow ? Object.keys(firstRow).filter(key => key !== 'id') : [];
  }
  ...
  canOpenDataList(msg: ChatDisplayMessage): boolean {
    const route = msg.action != null ? DATA_LIST_OPEN_ROUTES[msg.action] : undefined;
    return !!route && this.permissions.getGrantedPolicy(route.policy);
  }

  openDataList(msg: ChatDisplayMessage): void {
    const route = msg.action != null ? DATA_LIST_OPEN_ROUTES[msg.action] : undefined;
    if (route) this.router.navigateByUrl(route.path);
  }
