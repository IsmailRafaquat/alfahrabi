...
export class ShopUnitsComponent implements OnInit {
  ...
  create(): void { this.selected = undefined; this.unitHelpOpen = false; this.form.reset({ name: '', shortName: '', allowDecimal: false, isActive: true }); this.modalOpen = true; }
  edit(row: ShopUnitDto): void { this.service.get(row.id).subscribe(dto => { this.selected = dto; this.unitHelpOpen = false; this.form.patchValue(dto); this.modalOpen = true; }); }
  ...
  remove(row: ShopUnitDto): void { this.confirmation.warn('::ConfirmDeleteUnit', row.name).subscribe(s => { if (s !== Confirmation.Status.confirm) return; this.service.delete(row.id).subscribe({ next: () => { this.toaster.success('::UnitDeletedSuccessfully'); this.load(); }, error: e => this.toaster.error(e?.error?.error?.message || e?.message || '::UnexpectedError') }); }); }
