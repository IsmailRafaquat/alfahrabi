...
// School Management menu is temporarily hidden while the app is focused on Shop Management.
// Flip this back to true to restore the School Management menu tree.
const SHOW_SCHOOL_MENU = false;

function configureRoutes() {
  const routes = inject(RoutesService);
  routes.add([
    ...shopManagementRoutes,
    ...(SHOW_SCHOOL_MENU ? schoolManagementRoutes : []),
  ]);
}

const shopManagementRoutes = [
  {
    name: '::ShopManagement', iconClass: 'fas fa-store', order: 1,
...
const schoolManagementRoutes = [
  {
    name: '::SchoolManagement', iconClass: 'fas fa-school', order: 20,
